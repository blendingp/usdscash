...
<body id="page-top">
					...
					<div class="card shadow mb-4">						
						...
						<div class="col-lg-12">
							<div class="card-body">
									...
									<div class="row">
										...
										<div class="col-lg-6">
											<div class="form-group">
												...
												<pre><a href="/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/userDetail.do?idx=${info.uidx}">${info.name}</a>(<c:if test="${info.pname eq null}">없음</c:if>${info.pname})</pre>
											</div>
										</div>
										<div class="col-lg-3">
											<div class="form-group">
											...
											<div class="col-lg-12">
												<div class="form-group">
													...
													<c:forEach var="item" items="${fileList}">
														<pre><a href="/filePath/usdscash/photo/${item.saveNm}" download>${item.originNm}</a></pre>
													</c:forEach>
												</div>
											</div>
										</c:if>
								...
								<button type="button" onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/contact/contactDelete.do?idxs=${info.idx}'" class="btn btn-danger">
									문의 삭제
								</button>
								<c:if test="${info.confirm != '1'}">
	...
	function sendAnswer(type){
		...
		var url = "/usdscash/0nI0lMy6jAzAFRVe0DqLOw/contact/contactAnswer.do";
		if(type=="mail")
			url = "/usdscash/0nI0lMy6jAzAFRVe0DqLOw/contact/contactMailAnswer.do";
		console.log(url);
		$.ajax({
			type:'post',
			url : url,
			...
			success:function(data){
				...
				if(data.result == 'success'){
					alert("답변  완료되었습니다.");
					location.href = '/usdscash/0nI0lMy6jAzAFRVe0DqLOw/contact/contactList.do';
				}else{
					alert(data.msg);
					location.reload();
				}
	...
	function sendAnswerSms(){
		var data = $("#updateForm").serialize();
		var url = "/usdscash/0nI0lMy6jAzAFRVe0DqLOw/contact/contactAnswerSms.do";
		console.log(url);
		$.ajax({
			type:'post',
			url : url,
