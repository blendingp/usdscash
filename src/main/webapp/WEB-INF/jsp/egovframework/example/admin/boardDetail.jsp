...
<script src="/usdscash/se2/js/HuskyEZCreator.js" charset="utf-8" ></script>
...
function updateProcess(type){
	...
	var url = "/usdscash/0nI0lMy6jAzAFRVe0DqLOw/board/"+type+"Update.do";
	$.ajax({
		type:'post',
		url : url,
		data: data,
		success:function(data){
			if(data.result == 'success'){
				alert("글 수정이 완료되었습니다.");
				location.href="/usdscash/0nI0lMy6jAzAFRVe0DqLOw/board/"+type+"List.do";
			}else{
				alert(data.msg);
			}
		},
...
function delProcess(type , idx){
	...
	if(confirm("삭제하시겠습니까? 복구하실 수 없습니다.")){
		jQuery.ajax({
			type:'post',
			data:param,
			url:"/usdscash/0nI0lMy6jAzAFRVe0DqLOw/board/"+type+"Delete.do",
			success:function(data){
				if(data.result == 'success'){
					alert("삭제되었습니다.");
					location.href= "/usdscash/0nI0lMy6jAzAFRVe0DqLOw/board/"+type+"List.do";
