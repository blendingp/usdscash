			...
			<div id="content">
				...
				<div class="container-fluid">
					...
					<div class="card shadow mb-4">
						...
						<div class="card-body">
							<form action="/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/usdsTradeList.do"
								name="listForm" id="listForm">
								<input type="hidden" name="fileDown" id="fileDown" value="0" />
								<input type="hidden" name="pageIndex" value="1" />
								<input type="hidden" name="username" id="username" value="${username}" />
								<input type="hidden" name="searchIdx" id="searchIdx" value="${searchIdx}" />
								<input type="hidden" name="inverse" id="inverse" value="${inverse}" />
								...
								<div class="row">
									...
									<div class="col-lg-3">
										...
										<div class="form-group input-group">
											<select id="searchSelect" name="searchSelect"
												...
												<%-- <option value="symbol" <c:if test="${searchSelect eq 'symbol'}">selected</c:if>>Symbol</option> --%>
											</select>
											<input type="text" name="search" class="form-control" value="${search}" style="width: auto;">
											<button class="btn btn-default btn-sm" onclick="checkForm(2)" type="button">
										...
										<div class="form-group input-group">
											<span class="input-group-btn" style="white-space: normal;">
												<button class="btn btn-light btn-icon-split"
													...
													<c:set var="T" value="" />
													<c:if test="${inverse ne 'inverse'}">
														<c:set var="T" value="T" />
													</c:if>
													<button class="btn btn-light btn-icon-split"
														onclick="setSymbol('${coin}USD${T}')" type="button">${coin}</button>
												</c:forEach>
											</span>
										</div>
									</div> --%>
									...
									<div class="col-lg-1">
										...
										<div class="form-group input-group">
											<span class="input-group-btn">
												<button class="btn btn-info btn-sm" onclick="setOrder('buyDatetime')" type="button">시간</button>
												<button class="btn btn-success btn-sm" onclick="setOrder('result')" type="button">정산</button>
											</span>
										</div>
									</div>
									<div class="col-lg-1">
						...
						<div class="row" style="margin-left: 10px;">
							<div class="col-lg-4" style="width: fit-content;">
								<div class="form-group ">
									<pre style="margin-top: 20px;">&nbsp; 총 정산합  : ${feeResultSum.resultSum}</pre>
								</div>
							</div>
							<div class="col-lg-4" style="width: fit-content;">
								<div class="form-group ">
									<pre style="margin-top: 20px;">&nbsp; 테스트 총정산합 : ${testFeeResultSum.resultSum}</pre>
								</div>
							</div>
						</div>
						<div class="card-body">
										...
										<c:forEach var="item" items="${list}" varStatus="i">
											<tr style="background-color:${item.color}">
												...
												<c:if test="${item.pname ne null}">
													<td
														onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/userDetail.do?idx=${item.parentsIdx}'"
												...
												<td>
													...
													<c:if test="${item.isOpen eq false}">
														<fmt:formatNumber value="${item.liqPrice}" pattern="#,###.####" />
													</c:if>
												</td>
