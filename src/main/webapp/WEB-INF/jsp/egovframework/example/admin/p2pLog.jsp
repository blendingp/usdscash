					...
					<div class="row">
						<div class="col-lg-12">
							<div class="card shadow mb-4">
								...
								<div class="card-body">
									<form action="/usdscash/0nI0lMy6jAzAFRVe0DqLOw/p2p/p2pLog.do"
												...
												<c:forEach var="item" items="${list}" varStatus="i">
													<tr style="background-color:${item.color}">
														...
														<c:if test="${item.istest eq '1'}">
															<c:if test="${adminLevel ne 3}">
																<td onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/userDetail.do?idx=${item.useridx}'"
														...
														<c:if test="${item.istest ne '1'}">
															<c:if test="${adminLevel ne 3}">
																<td onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/userDetail.do?idx=${item.useridx}'" style="cursor: pointer;">
														...
														<td onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/p2p/p2pList.do?search=${item.tname}'" style="cursor: pointer;">
															${item.tname}
														</td>
														<td>
														...
														<td><c:if test="${item.unread != 0}">
																<button class="btn btn-danger btn-sm" type="button"
																	onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/p2p/p2pChat.do?midx=${item.idx}'">+${item.unread}</button>
															</c:if>
															<button class="btn btn-primary btn-sm statBtn"
																type="button"
																onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/p2p/p2pChat.do?midx=${item.idx}'">채팅</button>
														</td>
													</tr>
												</c:forEach>
											</tbody>
...
function depositCancel(idx , midx){
	if(confirm("입금취소하시겠습니까?")){
		$.ajax({
			type :'get',
			url : '/usdscash/0nI0lMy6jAzAFRVe0DqLOw/p2p/cancelDeposit.do?idx='+idx+'&midx='+midx,
...
function checkRequest(widx,stat){
    ...
    jQuery.ajax({                                
        type:"POST", 
        url : "/usdscash/0nI0lMy6jAzAFRVe0DqLOw/p2p/p2pWithdrawalProcess.do?widx="+widx+"&stat="+stat,
        dataType:"json",
...
function changeAlarm(idx, alarm, pkind){
	$.ajax({
		type :"post",
		dataType : "json" ,
		url : "/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/changeAlarm.do?idx="+idx+"&alarm="+alarm+"&kind=k"+pkind,
		success:function(data){
			if(data.result == "suc"){
				location.reload();
			}
...
function payConfirm(widx){
    jQuery.ajax({                                
        type:"POST", 
        url : "/usdscash/0nI0lMy6jAzAFRVe0DqLOw/p2p/payConfirm.do?widx="+widx,
        dataType:"json",
        date:{widx:widx},
        success : function(data) {              	
           if(data.result == "error")
...
function moneySend(midx){
	jQuery.ajax({
		type : "POST",
		data : {"midx" : midx},
		url : "/usdscash/0nI0lMy6jAzAFRVe0DqLOw/p2p/moneySend.do",
		dataType : "json",
		success : function(data) {
			if (data.result != "suc") {
				alert(data.msg);
