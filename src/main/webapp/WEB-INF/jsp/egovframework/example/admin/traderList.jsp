			...
			<div class="content">
				...
				<div class="container-fluid">
					<div class="card shadow mb-4">
						...
						<div class="card-body">
							<form
								action="/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trader/traderList.do"
								...
								<div class="table-responsive">
									<table class="table table-striped table-hover">
										...
										<c:forEach var="item" items="${traderList}" varStatus="i">
											<tbody>
												<td>
													<button type="button" onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trader/followerList.do?tidx=${item.tuseridx}'" class="btn btn-primary">목록</button>
												</td>
											
												<td style="cursor: pointer;"
													onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/userDetail.do?idx=${item.tuseridx}'">${item.name}&nbsp;</td>
												<td>
...
function changeTstat(idx , useridx, tstat ,istrader){
	if(confirm("변경하시겠습니까?")){
		$.ajax({
			type:'get',
			url:'/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trader/tstat.do?idx='+idx+'&useridx='+useridx+'&tstat='+tstat+'&istrader='+istrader,
...
function changeTotal(idx, useridx){
	...
	if(confirm("변경하시겠습니까?")){
		$.ajax({
			type:'get',
			url:'/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trader/total.do?idx='+idx+'&total='+total+'&useridx='+useridx,
			success:function(data){
				alert(data.msg);
				location.reload();
			},
...
function traderDelete(idx,useridx){
	if(confirm("삭제하시겠습니까? 복구하실수없습니다.")){
		$.ajax({
			type:'get',
			url:'/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trader/traderDelete.do?idx='+idx+'&useridx='+useridx,
			success:function(data){
				alert(data.msg);
				location.reload();
			},
...
function traderinfoUse(useridx,use,type){
	$.ajax({
		type:'get',
		url:'/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trader/traderinfoUse.do?useridx='+useridx+'&use='+use+'&type='+type,
		success:function(data){
			alert(data.msg);
			if(data.result=="suc")
				location.reload();
...
function traderinfoWrite(useridx){
	...
	$.ajax({
		type :'post',
		data : data,
		url:'/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trader/traderinfoWrite.do',
		success:function(data){
			alert(data.msg);
			if(data.result=="suc")
				location.reload();
