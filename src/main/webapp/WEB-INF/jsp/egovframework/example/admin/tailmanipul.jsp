...
<body id="page-top">
		            	...
		            	<div class="card shadow mb-4">
			                <div class="card-header py-3">
	                            <h6 class="m-0 font-weight-bold text-primary"><b>※ 동일 종목에 대해 꼬리달기가 진행 중일 때 중복 실행이 불가능합니다.</b>  
			                    	<input type="button" value="새로고침" onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/manipulation.do'">
			                    	<br>
			                    	<b>현재 실행중인 종목 : <span>${mList}</span></b><br>	                    	
			                    	<input type="button" style="float:right" value="로그" onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/log/manipullog.do'">
	...
	<script>
		...
		function setM(coin){
		    ...
		    if (confirm(msg)!=0) {
				...
				$.ajax({
					type :'post',
					data : data,
					url : '/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/manipultailProcess.do',
		...
		function getPositionData(){
			$.ajax({
				type :'post',
				url : '/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/getPositionData.do',
				success:function(data){
					var $trRate = $("#trRate");
					$trRate.empty();
		...
		function appendPositionList(coin,plist){
			...
			for(var i = 0; i < length; i++){
				...
				$id.append(
					"<tr><td style='cursor:pointer;' onclick=\"location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/userDetail.do?idx="+plist[i].userIdx+"'\">"+plist[i].name+"</td>" +
		...
		$.ajax({
			...
			success:function(data){
				if(data === 'ok') {
					$.ajax({
						type :'post',
						url : '/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/resetChart.do',
						success:function(data){
							alert(data.result);
						},
						error:function(e){
	...
	function resetM() {
		$.ajax({
			type :'post',
			url : '/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/resetMStatus.do',
			success:function(data){
				alert(data.msg);
				location.reload();
			},
