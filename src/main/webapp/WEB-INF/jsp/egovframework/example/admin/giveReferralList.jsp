		                ...
		                <div class="col-lg-12">
		                    <div class="panel panel-default">
		                        <div class="panel-body">
								   	<form action="/usdscash/0nI0lMy6jAzAFRVe0DqLOw/referral/giveReferralList.do" name="listForm" id="listForm">
...
$(function() {
	...
	$(".linkname").on("click", function() {
		...
		location.href="/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/userDetail.do?idx="+uidx;
	});
	...
	for(var i = 0; i < child.array.length; i++){
		if(Number(child.array[i].parentsIdx)==parentsIdx){
			...
			var divtext = "<tr class='influ_tablelcontent pidx"+child.array[i].idx+"' uidx='"+
				...
				date+"</td><td class='pEventSkip'><button class=\"btn-xs btn-default\" onclick=\"location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/referral/accumTradeLog.do?uidx="+
...
function gift(uidx){
	if(confirm("지급하시겠습니까?")){
		$.ajax({
			type :'post',
			data : {"uidx" : uidx},
			url : '/usdscash/0nI0lMy6jAzAFRVe0DqLOw/referral/accumReferralGift.do',
			success:function(data){
				console.log(data);
				if(data.result == "suc"){
					alert("지급 완료되었습니다.");
// 					location.href="/usdscash/0nI0lMy6jAzAFRVe0DqLOw/referral/giveReferralList.do";
...
function topGift(uidx){
	if(confirm("최상위 총판에게 지급하시겠습니까?")){
		$.ajax({
			type :'post',
			data : {"uidx" : uidx},
			url : '/usdscash/0nI0lMy6jAzAFRVe0DqLOw/referral/accumReferralGift_top.do',
			success:function(data){
				alert(data.msg);
				if(data.result == "suc"){
					let accumSum = Number(data.accumSum).toFixed(4);
