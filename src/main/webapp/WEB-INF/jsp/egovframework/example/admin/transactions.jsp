					...
					<div class="row">
						<div class="col-lg-12">
							<div class="card shadow mb-4">
								<div class="card-body">
									<form action="/usdscash/0nI0lMy6jAzAFRVe0DqLOw/transactions.do"
												...
												<c:forEach var="item" items="${list}" varStatus="i">
													<tr style="background-color:${item.color}">
														...
														<c:if test="${item.from ne 1 }">
															<td onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/userDetail.do?idx=${item.from}'" style="cursor: pointer;">${item.fromname}</td>
														</c:if>
														<td onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/userDetail.do?idx=${item.to}'" style="cursor: pointer;">${item.toname}</td>
														<td>${item.coin}</td>
...
function depositCancel(idx , midx){
	if(confirm("입금취소하시겠습니까?")){
		$.ajax({
			type :'get',
			url : '/usdscash/0nI0lMy6jAzAFRVe0DqLOw/cancelDeposit.do?idx='+idx+'&midx='+midx,
...
function checkRequest(widx,stat){	      
    jQuery.ajax({                                
        type:"POST", 
        url : "/usdscash/trade/withdrawalProcess.do?widx="+widx+"&stat="+stat,
        dataType:"json",
