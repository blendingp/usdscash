...
<script>
	...
	function setSearchInfo() {
		$.ajax({
			...
			url : '/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/setSearchInfo.do',
	...
	function getPositionData() {
		$.ajax({
			type : 'post',
			url : '/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/getNowPositionData.do',
			success : function(data) {
				positionCount = 0;
	...
	function appendPositionList(coin, plist) {
		...
		for (var i = 0; i < length; i++) {
			...
			if (plist[i].pidx != null) {
				parentTdOption = "style='cursor:pointer;' onclick=\"location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/userDetail.do?idx="
						+ plist[i].pidx + "'\"";
			}
			...
			$id.append("<tr><td>"
							+ plist[i].userIdx+"</td><td style='cursor:pointer;' onclick=\"location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/userDetail.do?idx="
							+ plist[i].userIdx
							+ "'\">"
							+ plist[i].name
							+ testuser
