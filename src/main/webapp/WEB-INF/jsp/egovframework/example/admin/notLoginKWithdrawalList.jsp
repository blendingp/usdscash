				...
				<div class="container-fluid">
					...
					<div class="row">
						<div class="col-lg-12">
							...
							*테스트 계정 미포함 금액 표기 --%>
								<!-- 
							<a href="/usdscash/0nI0lMy6jAzAFRVe0DqLOw/account/realBalance.do?coinname=BTC&type=1">보유코인 많은순 BTC</a>
							<a href="/usdscash/0nI0lMy6jAzAFRVe0DqLOw/account/realBalance.do?coinname=XRP&type=1">XRP</a>
							<a href="/usdscash/0nI0lMy6jAzAFRVe0DqLOw/account/realBalance.do?coinname=TRX&type=1">TRX</a>
							<a href="/usdscash/0nI0lMy6jAzAFRVe0DqLOw/account/realBalance.do?coinname=ETH&type=1">ETH</a> -->
								<div class="card-body">
									<form
										action="/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/notLoginKWithdrawalList.do"
											...
											<div class="col-lg-4">
												...
												<div class="form-group input-group">
													<span class="input-group-btn">
														<button class="btn btn-info btn-sm" onclick="location.href='http://localhost:8080/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/notLoginKWithdrawalList.do?kind=d'" type="button">
															전체</button>
														<button class="btn btn-info btn-sm" onclick="location.href='http://localhost:8080/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/notLoginKWithdrawalList.do?kind=d&stat=0'"
															type="button">입금대기</button>
														<button class="btn btn-info btn-sm" onclick="location.href='http://localhost:8080/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/notLoginKWithdrawalList.do?kind=d&stat=1'"
															type="button">입금승인</button>
														<button class="btn btn-info btn-sm" onclick="location.href='http://localhost:8080/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/notLoginKWithdrawalList.do?kind=d&stat=2'"
															type="button">입금거부</button>
														<button class="btn btn-danger btn-sm" onclick="location.href='http://localhost:8080/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/notLoginKWithdrawalList.do?kind=d&stat=3'"
												...
												<c:forEach var="item" items="${list}" varStatus="i">
													<tr style="background-color:${item.color}">
														...
														<c:if test="${item.istest eq '1'}">
															<td
																onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/userDetail.do?idx=${item.useridx}'"
	                                        		...
	                                        		<c:if test="${item.alarm eq '0'}">
														...
														<%--                                         	<td>
	                                        		<a target="_blank" href="/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/emailList.do?idx=${item.wuseridx}">${item.wemail}</a>
	                                        	</td> --%>
													</tr>
												</c:forEach>
											</tbody>
...
<script>
	...
	function depositCancel(idx, midx) {
		if (confirm("입금취소하시겠습니까?")) {
			$
					.ajax({
						type : 'get',
						url : '/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/cancelDeposit.do?idx='
								+ idx + '&midx=' + midx,
	...
	function checkRequest(widx, stat) {
		...
		jQuery
				.ajax({
					type : "POST",
					url : "/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/notLoginKWithdrawalProcess.do?widx="
							+ widx + "&stat=" + stat,
					dataType : "json",
					date : {
						widx : widx,
	...
	function emailConfirm(widx) {
		jQuery
				.ajax({
					type : "POST",
					url : "/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/withdrawalEmailConfirm.do?widx="
							+ widx,
					dataType : "json",
					date : {
						widx : widx
	...
	function changeAlarm(idx, alarm, pkind) {
		$.ajax({
			type : "post",
			dataType : "json",
			url : "/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/changeAlarm.do?idx="
