...
<body id="page-top">
							...
							<div class="card-body">
								<form name="updateForm" id="updateForm" method="post">
									...
									<div style="margin-bottom: 10px;">
										<%-- <button type="button"
											onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/orderList.do?searchSelect=idx&search=${info.idx}'"
											class="btn btn-primary">주문내역</button>
										<button type="button"
											onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/tradeList.do?searchSelect=idx&search=${info.idx}'"
											class="btn btn-primary">거래내역</button> --%>
										<button type="button"
											onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/usdsTradeList.do?searchSelect=idx&search=${info.idx}'"
											class="btn btn-primary">거래내역</button>
										<%-- <button type="button"
											onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/liqList.do?searchSelect=idx&search=${info.idx}'"
											class="btn btn-primary">청산내역</button>
										<c:if test="${adminLevel eq 1}">
											<button type="button"
												onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/positionList.do?search=${info.name}'"
												class="btn btn-primary">포지션내역</button>
										</c:if> --%>
										<c:if test="${project.coinDeposit eq true}">
											<button type="button"
												onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/transactions.do?pageIndex=1&searchSelect=idx&search=${info.idx}&label=%2B'"
												class="btn btn-primary">입금내역</button>
											<button type="button"
												onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/transactions.do?pageIndex=1&searchSelect=idx&search=${info.idx}&label=-'"
												class="btn btn-primary">출금내역</button>
										</c:if>
										<c:if test="${project.krwDeposit eq true}">
											<button type="button"
												onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/kWithdrawalList.do?pageIndex=1&kind=d&stat=0<c:if test="${info.istest eq 1}">&test=test</c:if>&searchSelect=idx&search=${info.idx}'"
												class="btn btn-primary">입금내역(한화)</button>
											<button type="button"
												onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/kWithdrawalList.do?pageIndex=1&kind=w&stat=0<c:if test="${info.istest eq 1}">&test=test</c:if>&searchSelect=idx&search=${info.idx}'"
												class="btn btn-primary">출금내역(한화)</button>
										</c:if>
										<button type="button"
											onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/contact/contactList.do?searchSelect=m.phone&search=${info.phone}'"
												...
												<div class="form-group">
													...
													<div class="form-group input-group">
														...
														<c:if test="${info.fkey ne null}">
															<span class="input-group-btn">
																<button type="button" onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/kycInfo.do?idx=${info.idx}'" class="btn btn-info">확인</button>
															</span>
														</c:if>
														<span class="input-group-btn">
	...
	총1 = 본인 60% - 총2 40% = 20%
													...
													<div class="col-lg-4">
														<div class="form-group">
															...
															<c:if test="${traderInfo.timg ne null}">
																<pre><img src="/filePath/usdscash/photo/${traderInfo.timg}" loading="lazy"><br>${traderInfo.timg}</pre>
															</c:if>
				...
				<button type="button" onclick="location.href='/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/userList.do'" class="btn btn btn-secondary">
					목록
				</button>
				<br /> <br />
			</div>
	...
	function userDanger(idx , danger){
		...
		if(confirm(txt)){
			...
			$.ajax({
				type :'post',
				url : '/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/userDanger.do',
				data : data ,
	...
	function change(type , kind ,self){
		$.ajax({
			...
			url:'/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/changeLevelOption.do',
			success:function(data){
	...
	function updateLevel(idx, level){
			$.ajax({
				...
				url:'/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/updateLevel.do',
				dataType:'json',
				success:function(data){
					alert(data.msg);
					if(data.result == 'success'){
	...
	function updatePoint(idx , kind){
		...
		$.ajax({
			...
			url:'/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/updatePoint.do',
			dataType:'json',
			success:function(data){
				alert(data.msg);
				if(data.result == 'success'){
	...
	function depositUSDT(idx){
		...
		$.ajax({
			...
			url:'/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/depositUSDT.do',
			dataType:'json',
			success:function(data){
				alert(data.msg);
				if(data.result == 'success'){
	...
	function updateCommissionRate(idx){
		...
		$.ajax({
			...
			url:'/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/updateCommissionRate.do',
			dataType:'json',
			success:function(data){
				alert(data.msg);
				if(data.result == 'success'){
	...
	function ipBan(idx){
		...
		$.ajax({
			type:'post',
			data:{"idx" : idx , "ip" : ip},
			url:'/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/ipBan.do',
			dataType:'json',
			success:function(data){
				alert(data.msg);
				if(data.result == 'success'){
	...
	function releaseBan(idx){
		...
		$.ajax({
			type:'post',
			data:{"idx" : idx , "ip" : ip},
			url:'/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/releaseBan.do',
			dataType:'json',
			success:function(data){
				alert(data.msg);
				if(data.result == 'success'){
	...
	function userBan(idx){
		$.ajax({
			type:'post',
			data:{"idx" : idx},
			url:'/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/userBan.do',
			dataType:'json',
			success:function(data){
				alert(data.msg);
				if(data.result == 'success'){
	...
	function releaseUserBan(idx){
		$.ajax({
			type:'post',
			data:{"idx" : idx},
			url:'/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/releaseUserBan.do',
			dataType:'json',
			success:function(data){
				alert(data.msg);
				if(data.result == 'success'){
...
// 	}
	...
	function listChange(node,kind){
		...
		if(kind == "deposit"){
			...
			$("#listlink").attr("href","/usdscash/0nI0lMy6jAzAFRVe0DqLOw/transactions.do?searchIdx="+"${info.idx}"+"&label=%2B"+"&username="+"${info.name}");
			$("#listlink").html("입금내역 전체보기");
		}
		else if(kind == "withdraw"){
			$("#wlist").css("display","flex");
			$("#listlink").attr("href","/usdscash/0nI0lMy6jAzAFRVe0DqLOw/transactions.do?searchIdx="+"${info.idx}"+"&label=-"+"&username="+"${info.name}");
			$("#listlink").html("출금내역 전체보기");
		}
		else if(kind == "trade"){
			$("#tlist").css("display","flex");
			$("#listlink").attr("href","/usdscash/0nI0lMy6jAzAFRVe0DqLOw/trade/tradeList.do?searchIdx="+"${info.idx}"+"&username="+"${info.name}");
	...
	function updateInfo(kind){ // all - 전체변경, 혹은 하나만 변경
		...
		$.ajax({
			type :'post',
			data : data,
			url : '/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/updateInfo.do?kind='+$("#kind").val(),
	...
	function memberDelete(idx){ // all - 전체변경, 혹은 하나만 변경
		...
		$.ajax({
			type:'post',
			data:{"idx" : idx},
			url:'/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/deleteUser.do',
			dataType:'json',
			success:function(data){
				alert(data.msg);
				if(data.result == 'success'){
	...
	function parentChange(parentIdx){
		...
		$.ajax({
			...
			url:'/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/parentChange.do',
			dataType:'json',
			success:function(data){
				alert(data.msg);
				if(data.result == 'suc')
	...
	function changeJstat(idx , kind){
		if(confirm("변경하시겠습니까?")){
			$.ajax({
				type : 'get',
				url : '/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/changeJstat.do?idx='+idx+'&jstat='+kind,
				success:function(data){
					alert(data.msg);
					location.reload();
				}
	...
	function setKrCode(idx){
		if(confirm("변경하시겠습니까?")){
			$.ajax({
				type : 'get',
				url : '/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/setKrCode.do?idx='+idx,
				success:function(data){
					alert(data.msg);
					location.reload();
				}
	...
	function vConfirm(idx,vcf){
		if(confirm("변경하시겠습니까?")){
			$.ajax({
				type : 'get',
				url : '/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/setVConfirm.do?idx='+idx+"&vConfirm="+vcf,
				success:function(data){
					alert(data.msg);
					location.reload();
				}
	...
	function kycConfirm(idx , cf){
		...
		$.ajax({
			type : 'get',
			url : '/usdscash/0nI0lMy6jAzAFRVe0DqLOw/user/kycConfirm.do?idx='+idx+'&confirm='+cf+"&text="+text,
			success:function(data){
				alert(data.msg);
				location.reload();
			}
