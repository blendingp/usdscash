...
<!-- 							</div> -->
							...
							<div class="warp1">
								...
								<c:if test="${info.level ne 'user'}">
									...
									<a href="/usdscash/infl/login.do" target="_blank" class="button-38 w-button">PARTNER</a>
								</c:if>
							</div>
...
<%-- 						<a href="/usdscash/user/myInfo.do" class="button-21-copy w-button"><spring:message code="wallet.cancel"/></a> --%>
<!-- 					</div> -->
					<img src="/usdscash/webflow/images2/wallet-icon4.png" loading="lazy" alt="" class="asset_img">
				</div>
			</div>
		</form>
		...
		<div class="registpop" id="telegramPop" style="display: none">
			<div class="registpop_blcok">
				<div class="pop_exist" style="cursor: pointer;">
					<img onclick="javascript:$('#telegramPop').css('display', 'none')" src="/usdscash/webflow/images2/wx.png" loading="lazy" alt="" class="image-38">
				</div>
				<div class="poptitle">
					<div class="title6">
	...
	<script>
		...
		function updateBankInfo(id) {
			$.ajax({
				...
				dataType : 'json',
				url : '/usdscash/user/updateBankInfo.do',
		...
		function sendRequestEmail() {
			...
			$.ajax({
				...
				dataType : 'json',
				url : '/usdscash/user/verificationEmailUpdate.do',
				success : function(data) {
					alert(data.msg);
		...
		function emailCodeCheck() {
			...
			$.ajax({
				...
				dataType : 'json',
				url : '/usdscash/user/verificationEmailConfirm.do',
				success : function(data) {
					alert(data.msg);
					if (data.result == "success") {
						checkEmail = true;
		...
		function updateInfo() {
			...
			$.ajax({
				type : 'post',
				data : data,
				url : '/usdscash/user/updateMyInfo.do',
				success : function(data) {
					alert(data.msg);
					if (data.result == 'success') {
						location.reload();
		...
		function getTelegramCode(){
			$.ajax({
				type : 'post',
				url : '/usdscash/user/getTelegramCode.do',
				success : function(data) {
					if (data.result == "suc") {
						$("#tCode").val(data.code);
						vCountStart(5);
