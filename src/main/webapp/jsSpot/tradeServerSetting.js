let wsUriToWeb = "wss://"+servername+":"+serverport+"/usdscash/websocket/echo.do"; //주소 확인!!
if(servername == "localhost")
	wsUriToWeb = "ws://"+servername+":"+serverport+"/usdscash/websocket/echo.do";
...
function onAPIMessage(evt) {
	...
	if (stream.slice(-8) === '@depth20') {
		try {
			...
			var hqSum = 0;
			for (m = 0; m <= 6; m++) {
				var hq = Number(parseFloat(jdata.data.a[m][1]).toFixed(qtyfixnum));
				hqSum += hq;
				$(".ordertable .longhoga:eq(" + (6-m) + ") .hp.long").html(parseFloat(jdata.data.a[m][0]).toFixed(fixnum));
				$(".ordertable .longhoga:eq(" + (6-m) + ") .hq").html(hq);
				$(".ordertable .longhoga:eq(" + (6-m) + ") .hsum").html(hqSum.toFixed(qtyfixnum));
			...
			for (m = 0; m <= 6; m++) {
				var hq = Number(parseFloat(jdata.data.a[m][1]).toFixed(qtyfixnum));
				hqSum += hq;
				$(".ordertable .shorthoga:eq(" + m + ") .hp.short").html( parseFloat(jdata.data.b[m][0]).toFixed(fixnum));
				$(".ordertable .shorthoga:eq(" + m + ") .hq").html(hq);
				$(".ordertable .shorthoga:eq(" + m + ") .hsum").html(hqSum.toFixed(qtyfixnum));
...
function onMessageToWeb(evt) { // 받은 메세지를 보여준다
	...
	try {
		...
		if (obj.protocol == "warning") {
			...
			location.href = "/usdscash/showRest.do";
		...
		if (obj.protocol == "showPopup" && obj.msg == "wrongUser") {
			...
			///------------------------실제사용
			if (obj.protocol == "order set") {//주문 넣은 후 주문내역 추가				
				setOrder(obj);
			}
			...
			if (obj.protocol == "showPopup") {
				...
				if(obj.msg=="kycPop") 
					location.href='/usdscash/user/kycCenter.do';
				else
					showPopupFromServer(obj.msg, obj.level);
			}
			...
			if (obj.protocol == "sendTradeBuy") {
				var cnum = getCoinNum(obj.symbol);
				spotLongEntryPriceList[cnum].push(obj.entryPrice);
			}
